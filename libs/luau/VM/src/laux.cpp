...
// HR_BEGIN - Modified to better report line numbers when errors happen from within C#

int luaL_where_impl(lua_State* L, int level)
{
    lua_Debug ar;
    if (lua_getinfo(L, level, "sl", &ar) && ar.currentline > 0 && ar.source[0] != '@')
    {
...
void luaL_where(lua_State* L, int level)
{
    if (level < 0)
    {
        if (luaL_where_impl(L, level))
            return;
    }
    else
    {
        for (; unsigned(level) < unsigned(L->ci - L->base_ci); level++)
            if (luaL_where_impl(L, level))
                return;
    }
