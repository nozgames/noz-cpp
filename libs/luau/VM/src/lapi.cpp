...
// HR_BEGIN
int lua_rawpcall(lua_State* L, Pfunc func, void* u, int nargs, int nresults, int errfunc)
{
    api_checknelems(L, nargs);
    api_check(L, L->status == 0);
    checkresults(L, nargs, nresults);

    ptrdiff_t errFuncPtr = 0;
    if (errfunc != 0)
    {
        StkId o = index2addr(L, errfunc);
        api_checkvalidindex(L, o);
        errFuncPtr = savestack(L, o);
    }

    int status = luaD_pcall(L, func, u, savestack(L, L->top - nargs), errFuncPtr);
