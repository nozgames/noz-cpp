...
void main() {
    vec4 color = texture(mainTexture, f_uv);

    // Convert to grayscale using luminance weights
    float gray = dot(color.rgb, vec3(0.299, 0.587, 0.114));

    // Mix between original color and grayscale based on desaturation amount
    vec3 result = mix(color.rgb, vec3(gray), params.desaturation);

    // Apply brightness adjustment (optional)
    result *= params.brightness;
