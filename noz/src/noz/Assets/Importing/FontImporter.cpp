...
namespace noz {
  ...
  std::vector<FT_Glyph> FT_GetGlyphs(FT_Face face, const FontMeta& options, wchar_t* codes, size_t numCodes, noz_int32 sdf_scale, noz_int32 sdf_scan) {
  ...
  static void RenderGlyphs(FT_Face face, Image* image, std::vector<FT_Glyph>& glyphs, const FontMeta& options) {    
    noz_int32 image_pitch = image->GetWidth();
    ...
    for(size_t g=0; g<glyphs.size(); g++) {
      ...
      noz_byte* out = (noz_byte*)image->GetBuffer();
      noz_byte* in = face->glyph->bitmap.buffer;
      for(noz_int32 r=0; r<face->glyph->bitmap.rows; r++) {
        memcpy(
          out + image_pitch * (r+glyph.packed.y+1) + glyph.packed.x + 1,
  ...
  static void RenderSDFGlyphs(FT_Face face, Image* image, noz_int32 sdf_scale, noz_int32 sdf_scan, std::vector<FT_Glyph>& glyphs) {    
    noz_byte* buffer = (noz_byte*)image->GetBuffer();
...
Asset* FontImporter::Import (const String& path) {
  FileStream fs;
  if(!fs.Open(path, FileMode::Open)) {
    return nullptr;
  }

  Object* o = JsonDeserializer().Deserialize(&fs);
  if(!o->IsType(typeof(FontMeta))) {
    delete o;
    return nullptr;
  }

  FontMeta* meta = (FontMeta*)o;

  // Open stream to the source TTF  
  fs.Close();
  if(!fs.Open(Path::Combine(Path::GetDirectoryName(path), meta->source), FileMode::Open)) {
    delete meta;
    return nullptr;
  }

  Font* font = Import(&fs,*meta);
...
Font* FontImporter::Import(Stream* ttf, const FontMeta& options) {
  noz_assert((Stream*)ttf);

  noz_int32 sdf_scale = options.sdf_scale ? options.sdf_scale : 1;
  noz_int32 sdf_scan  = options.sdf_scale ? options.sdf_scan : 0;
  ...
  noz_float height = (noz_float)options.height;
  if(options.sdf) {
    height -= 8.0f;
  }
  ...
  if(!FT_LoadFace(library, &face, (noz_float)size, options.sdf_scale, ttf)) {
  ...
  std::vector<FT_Glyph> ft_glyphs = FT_GetGlyphs(face,options,codes,num_codes,sdf_scale,0);
  ...
  Image* image = new Image(packer.GetSize().w,packer.GetSize().h,options.sdf_scale ? ImageFormat::SDF : ImageFormat::A8);
  image->Lock();

  // Render the glyphs to the sheet in their respective locations.
  if(options.sdf) {
    RenderSDFGlyphs(face,image,options.sdf_scale,options.sdf_scan,ft_glyphs);
  } else {
    RenderGlyphs(face,image,ft_glyphs,options);
  }
  ...
  font->image_ = image;
  font->distance_field_ = options.sdf_scale != 0;
