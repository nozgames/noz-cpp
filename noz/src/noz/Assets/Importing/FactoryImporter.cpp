...
#include "FactoryImporter.h"

using namespace noz;


Asset* FactoryImporter::Import (const String& path) {
  ...
  if(def.factory_type_ == nullptr) {
    Console::WriteLine("%s: error: missing or invalid FactoryType", path.ToCString());
    return nullptr;
  }
  ...
  if(nullptr == factory) {
    Console::WriteLine("%s: error: failed to create instance of FactoryType '%s'", path.ToCString(),def.factory_type_->GetQualifiedName().ToCString());
    return nullptr;
  }
  ...
  for(noz_uint32 i=0,c=def.objects_.size(); i<c; i++) {
    ...
    if(p==nullptr) {
      Console::WriteLine("%s: warning: FactoryType '%s' does not contain a property named '%s'", 
        path.ToCString(),def.factory_type_->GetQualifiedName().ToCString(),odef->property_.ToCString());
      continue;
    }
    ...
    if(!p->GetObjectType()->IsCastableTo(typeof(SerializedObject))) {
      Console::WriteLine("%s: warning: property '%s' of FactoryType '%s' must be of type 'ObjectPtr<SerializedObject>'", 
        path.ToCString(),odef->property_.ToCString(),def.factory_type_->GetQualifiedName().ToCString());
      continue;
    }
