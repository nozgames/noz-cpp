...
namespace noz::editor {
    ...
    static void UpdateScale() {
        ...
        float current_distance = Length(g_workspace.mouse_world_position - g_scale.options.origin);
    ...
    static void DrawScale() {
        ...
        DrawLine(g_workspace.mouse_world_position, g_scale.options.origin, SCALE_TOOL_WIDTH);
        BindColor(COLOR_ORIGIN);
        DrawVertex(g_workspace.mouse_world_position, SCALE_TOOL_CENTER_SIZE);
    }
    ...
    void BeginScaleTool(const ScaleToolOptions& options) {
        ...
        g_scale.delta_scale = VEC2_ONE;
        g_scale.start_distance = Length(g_workspace.mouse_world_position - options.origin);
