...
namespace noz::editor {
    ...
    static void UpdateVertexWeightTool() {
        ...
        float delta = (g_workspace.mouse_position.y - g_workspace.drag_position.y) / (g_workspace.dpi * WEIGHT_TOOL_SIZE);
    ...
    static void DrawVertexWeightTool() {
        ...
        for (int i=0; i<g_weight.options.vertex_count; i++) {
            ...
            DrawMesh(g_workspace.circle_mesh, TRS(v.position, 0, VEC2_ONE * CIRCLE_CONTROL_OUTLINE_SIZE * g_workspace.zoom_ref_scale));
        }
        ...
        for (int i=0; i<g_weight.options.vertex_count; i++) {
            ...
            DrawMesh(g_workspace.circle_mesh, TRS(v.position, 0, VEC2_ONE * CIRCLE_CONTROL_SIZE * g_workspace.zoom_ref_scale));
            BindColor(COLOR_VERTEX_SELECTED);
            DrawMesh(g_workspace.arc_mesh[arc], TRS(v.position, 0, Vec2{-1, 1} * CIRCLE_CONTROL_SIZE * g_workspace.zoom_ref_scale));
        }
