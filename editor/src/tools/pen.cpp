...
static void UpdatePenTool() {
    ...
    if (g_pen_tool.point_count >= 3) {
        if (HitTestVertex(g_pen_tool.points[0].position + m->position, g_view.mouse_world_position, PEN_HIT_TOLERANCE)) {
    ...
    if (!g_pen_tool.hovering_first_point) {
        int vertex_index = HitTestVertex(m, g_view.mouse_world_position, PEN_HIT_TOLERANCE);
        if (vertex_index >= 0) {
