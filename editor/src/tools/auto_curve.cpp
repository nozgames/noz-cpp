...
namespace noz::editor {
    ...
    void BeginAutoCurveTool(MeshDocument* mdoc) {
#if 0
        ...
        for (u16 face_index = 0; face_index < frame->face_count; face_index++) {
            ...
            for (int i = 0; i < f.vertex_count; i++) {
                ...
                int edge_index = GetEdge(mdoc, v0, v1);
                if (edge_index < 0)
                    continue;
        ...
        Vec2 centroid_world = g_auto_curve.centroid + mdoc->position;
        g_auto_curve.initial_distance = Length(g_workspace.mouse_world_position - centroid_world);
