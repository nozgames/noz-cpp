...
namespace noz::editor {
    ...
    static void InitConfig() {
        ...
        g_editor.output_path = fs::absolute(project_path / fs::path(g_editor.config->GetString("editor", "output_path", "assets"))).string();
        g_editor.unity_path = fs::absolute(project_path / fs::path(g_editor.config->GetString("editor", "unity_path", "./assets/noz")));
        g_editor.save_dir = g_editor.config->GetString("editor", "save_path", "assets");
        g_editor.project_path = project_path.string();

        // Atlas settings from [atlas] section
        g_editor.atlas.size = g_editor.config->GetInt("atlas", "size", ATLAS_DEFAULT_SIZE);
        g_editor.atlas.dpi = g_editor.config->GetInt("atlas", "dpi", ATLAS_DEFAULT_DPI);
        g_editor.atlas.padding = g_editor.config->GetInt("atlas", "padding", ATLAS_RECT_PADDING);
        Set(g_editor.atlas.prefix, g_editor.config->GetString("atlas", "prefix", "atlas").c_str());
        std::string filter = g_editor.config->GetString("atlas", "filter", "linear");
        g_editor.atlas.filter = (filter == "nearest" || filter == "point")
            ? TEXTURE_FILTER_NEAREST : TEXTURE_FILTER_LINEAR;
        g_editor.atlas.antialias = g_editor.config->GetBool("atlas", "antialias", false);
    ...
    void BeginTool(const Tool& tool) {
        ...
        if (g_editor.tool.input == nullptr)
            g_editor.tool.input = g_workspace.input_tool;
    ...
    static void InitPalettes() {
        for (auto& palette_key : g_editor.config->GetKeys("palettes")) {
            std::string palette_value = g_editor.config->GetString("palettes", palette_key.c_str(), nullptr);
            Tokenizer tk;
        ...
        g_palette_texture_name = GetName(g_editor.config->GetString("editor", "palette", "palette").c_str());
