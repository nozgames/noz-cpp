...
bool UpdateHelp () {
    ...
    if (g_view.state == VIEW_STATE_DEFAULT) {
        WorkspaceHelp();
    } else if (g_view.state == VIEW_STATE_EDIT) {
        assert(g_editor.editing_asset);
        if (g_editor.editing_asset->vtable.editor_help)
            g_editor.editing_asset->vtable.editor_help();
    }
