...
static AtlasData* CreateManagedAtlas() {
    ...
    AtlasData* atlas = static_cast<AtlasData*>(CreateAssetData(atlas_path));
    if (atlas) {
...
void RegisterManagedAtlas(AtlasData* atlas) {
    if (!atlas) return;

    // Check if already registered
    for (AtlasData* existing : g_managed_atlases) {
...
void UnregisterManagedAtlas(AtlasData* atlas) {
...
bool IsManagedAtlas(AtlasData* atlas) {
    if (!atlas) return false;
    return IsManagedAtlasName(atlas->name);
}

AtlasData* GetManagedAtlas(int index) {
...
bool NeedsAtlasAssignment(MeshData* mesh) {
    ...
    AtlasData* existing = FindAtlasForMesh(mesh->name);
    if (existing) {
...
AtlasData* AutoAssignMeshToAtlas(MeshData* mesh) {
    ...
    AtlasData* new_atlas = CreateManagedAtlas();
    if (!new_atlas) {
