...
namespace noz::editor {
    ...
    static bool CollectBuildAsset(Document* doc, void* user_data) {
        BuildCollector* collector = static_cast<BuildCollector*>(user_data);

        if (doc->editor_only)
            return true;

        if (doc->def->type != collector->type)
            return true;

        std::string type_upper = ToString(doc->def->type);
        Upper(type_upper.data(), (u32)type_upper.size());

        std::string name_upper = doc->name->value;
    ...
    static void WriteBuildAsset(FILE* file, Document* a, const char* extension, const char* suffix) {
        std::string type_upper = ToString(a->def->type);
        Upper(type_upper.data(), (u32)type_upper.size());
    ...
    void Build() {
        ...
        for (int type = 0; type < ASSET_TYPE_COUNT; type++) {
            ...
            for (int doc_index = 0, doc_count = GetDocumentCount(); doc_index < doc_count; doc_index++)
                CollectBuildAsset(GetDocument(doc_index), &collector);
