...
namespace noz::editor {
    ...
    extern Document* CreateAssetDataForImport(const std::filesystem::path& path);
    ...
    struct Importer {
        ...
        std::set<Document*> pending_imports;  // Track assets with pending import jobs
        noz::Task post_import_task;
    };

    static Importer g_importer = {};

    static void QueueImport(Document* a, bool force = false) {
        fs::path path = a->path.value;
        if (!fs::exists(path))
            return;

        const DocumentDef* doc_def = a->def;
        if (!doc_def)
            return;
    ...
    void QueueImport(const fs::path& path) {
        ...
        Document* a = FindDocument(doc_def->type, asset_name);
        if (!a) {
    ...
    static void ExecuteImportJob(ImportJob* job) {
        ...
        const DocumentDef* doc_def = job->asset->def;
        assert(doc_def);

        fs::path target_dir =
            fs::path(g_editor.output_path) /
            ToString(job->asset->def->type) /
        ...
        try {
            doc_def->import_func(job->asset, target_dir_lower, g_editor.config, meta);
    ...
    static void CleanupOrphanedAssets() {
        ...
        for (u32 i=0, c=GetDocumentCount(); i<c; i++)
            source_paths.insert(GetTargetPath(GetDocument(i)));
    ...
    static bool UpdateTasks() {
        ...
        g_importer.post_import_task = noz::CreateTask({
            .run = [](noz::Task) -> void* {
                GenerateAssetManifest(g_editor.output_path, g_editor.config);
                return noz::TASK_NO_RESULT;
            },
            .name = "post_import"
        });
    ...
    void ReimportAll() {
        for (u32 i=0, c=GetDocumentCount(); i<c; i++)
            QueueImport(GetDocument(i), true);  // force reimport
    }
    ...
    void InitImporter() {
        ...
        if (g_editor.config->HasKey("manifest", "generate_lua"))
            g_importer.manifest_lua_path = fs::weakly_canonical(fs::path(g_editor.project_path)
                / g_editor.config->GetString("manifest", "generate_lua", "./assets/lua/global.lua"));

        if (g_editor.config->HasKey("manifest", "generate_cpp"))
            g_importer.manifest_cpp_path = fs::weakly_canonical(fs::path(g_editor.project_path)
                / g_editor.config->GetString("manifest", "generate_cpp", "./src/game_assets.cpp"));
