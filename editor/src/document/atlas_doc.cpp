...
extern void InitAtlasEditor(AtlasDocument* atlas);
...
static void DrawAtlasData(Document* a) {
    assert(a);
...
static void AllocAtlasDataImpl(Document* a) {
    assert(a->type == ASSET_TYPE_ATLAS);
    AtlasDocument* atlas = static_cast<AtlasDocument*>(a);
...
static void DestroyAtlasData(Document* a) {
    AtlasDocument* atlas = static_cast<AtlasDocument*>(a);
...
static void CloneAtlasData(Document* a) {
    assert(a->type == ASSET_TYPE_ATLAS);
    AtlasDocument* atlas = static_cast<AtlasDocument*>(a);
...
AtlasRect* FindRectForMesh(AtlasDocument* atlas, const Name* mesh_name) {
    AtlasDataImpl* impl = atlas->impl;
...
AtlasRect* AllocateRect(AtlasDocument* atlas, MeshDocument* mesh) {
    AtlasDataImpl* impl = atlas->impl;
...
void FreeRect(AtlasDocument* atlas, AtlasRect* rect) {
    if (rect) {
        // Clear the mesh's atlas reference
        if (rect->mesh_name) {
            Document* mesh_asset = GetDocument(ASSET_TYPE_MESH, rect->mesh_name);
            if (mesh_asset) {
                MeshDocument* mesh = static_cast<MeshDocument*>(mesh_asset);
...
void ClearRectPixels(AtlasDocument* atlas, const AtlasRect& rect) {
    AtlasDataImpl* impl = atlas->impl;
...
void ClearAllRects(AtlasDocument* atlas) {
    AtlasDataImpl* impl = atlas->impl;
    for (int i = 0; i < impl->rect_count; i++) {
        // Clear the mesh's atlas reference
        if (impl->rects[i].valid && impl->rects[i].mesh_name) {
            Document* mesh_asset = GetDocument(ASSET_TYPE_MESH, impl->rects[i].mesh_name);
            if (mesh_asset) {
                MeshDocument* mesh = static_cast<MeshDocument*>(mesh_asset);
                mesh->impl->atlas = nullptr;
            }
...
static void EnsurePixelBuffer(AtlasDocument* atlas) {
    AtlasDataImpl* impl = atlas->impl;
    if (!impl->pixels)
...
void SyncAtlasTexture(AtlasDocument* atlas) {
    AtlasDataImpl* impl = atlas->impl;
...
void RegenerateAtlas(AtlasDocument* atlas, PixelData* pixels) {
    AtlasDataImpl* impl = atlas->impl;
...
void RenderMeshToAtlas(AtlasDocument* atlas, MeshDocument* mesh, AtlasRect& rect, bool update_bounds) {
    EnsurePixelBuffer(atlas);
...
void RebuildAtlas(AtlasDocument* atlas) {
    AtlasDataImpl* impl = atlas->impl;
    ...
    for (int i = 0; i < rect_info_count; i++) {
        Document* mesh_asset = GetDocument(ASSET_TYPE_MESH, rect_infos[i].name);
        if (!mesh_asset) {
...
Vec2 GetAtlasUV(AtlasDocument* atlas, const AtlasRect& rect, const Bounds2& mesh_bounds, const Vec2& position) {
    AtlasDataImpl* impl = atlas->impl;
...
void GetExportQuadGeometry(AtlasDocument* atlas, const AtlasRect& rect,
...
void GetTightQuadGeometry(AtlasDocument* atlas, const AtlasRect& rect,
    Vec2* out_min, Vec2* out_max,
    float* out_u_min, float* out_v_min, float* out_u_max, float* out_v_max) {
...
Mesh* GetAtlasOutlineMesh(AtlasDocument* atlas) {
...
static void SaveAtlasData(Document* a, const std::filesystem::path& path) {
    AtlasDocument* atlas = static_cast<AtlasDocument*>(a);
    AtlasDataImpl* impl = atlas->impl;
...
static void ParseRect(AtlasDocument* atlas, Tokenizer& tk) {
    AtlasDataImpl* impl = atlas->impl;
...
static void LoadAtlasData(Document* a) {
    AtlasDocument* atlas = static_cast<AtlasDocument*>(a);
    AtlasDataImpl* impl = atlas->impl;
...
static void PostLoadAtlasData(Document* a) {
    AtlasDocument* atlas = static_cast<AtlasDocument*>(a);
    AtlasDataImpl* impl = atlas->impl;
...
void InitAtlasDocument(Document* a) {
    assert(a);
    assert(a->type == ASSET_TYPE_ATLAS);
...
Document* NewAtlasDocument(const std::filesystem::path& path) {
    ...
    AtlasDocument* atlas = static_cast<AtlasDocument*>(CreateAssetData(full_path));
    LoadAtlasData(atlas);
    return atlas;
}
