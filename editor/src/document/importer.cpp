...
static const DocumentImporter* FindImporter(const fs::path& ext) {
    const DocumentDef* info = FindDocumentDef(ext.string().c_str());
    if (info)
        return &info->importer;
    return nullptr;
}

bool InitImporter(Document* a) {
    fs::path path = a->path.value;
    if (!fs::exists(path))
        return false;

    const DocumentImporter* importer = FindImporter(path.extension());
    if (!importer)
        return false;
...
static void QueueImport(Document* a, bool force = false) {
    ...
    const DocumentDef* type_info = GetDocumentDef(a->type);
    if (!type_info)
        return;
...
void QueueImport(const fs::path& path) {
    ...
    Document* a = GetDocument(importer->type, asset_name);
    if (!a) {
...
static void ExecuteImportJob(ImportJob* job) {
    ...
    const DocumentDef* type_info = GetDocumentDef(job->asset->type);
    assert(type_info);
