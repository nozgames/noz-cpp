...
namespace noz::editor {
    ...
    extern MeshFrameData* GetCurrentFrame(MeshDocument* m);
    extern void SetCurrentFrame(MeshDocument* m, int frame_index);
    extern int GetFrameCount(MeshDocument* m);
    extern void AddFrame(MeshDocument* m, int after_index);
    extern void DeleteFrame(MeshDocument* m, int frame_index);
    extern void CopyFrame(MeshDocument* m, int src_frame, MeshFrameData* dst);
    extern void PasteFrame(MeshDocument* m, int dst_frame, const MeshFrameData* src);

    inline MeshFrameData* GetCurrentFrame(MeshDocument* m) {
        return &m->frames[m->current_frame];
    }

    inline MeshFrameData* GetFrame(MeshDocument* m, int frame) {
        return &m->frames[frame];
    }

    inline int GetFrameCount(MeshDocument* m) {
    ...
    inline int HitTestVertex(MeshDocument* m, const Vec2& position, float size_mult=1.0f) {
        return HitTestVertex(m, Translate(m->position), position, size_mult);
    }
    extern Vec2 HitTestSnap(MeshDocument* m, const Vec2& position);
    extern int HitTestVertex(const Vec2& position, const Vec2& hit_pos, float size_mult=1.0f);
    extern int HitTestEdge(MeshDocument* m, const Mat3& transform, const Vec2& position, float* where=nullptr, float size_mult=1.0f);
    inline int HitTestEdge(MeshDocument* m, const Vec2& position, float* where=nullptr, float size_mult=1.0f) {
    ...
    extern void DeleteSelectedFaces(MeshDocument* m);
    extern void SetHeight(MeshDocument* m, int index, float height);
    extern int SplitEdge(MeshDocument* m, int edge_index, float edge_pos, bool update=true);
    extern int SplitTriangle(MeshDocument* m, int triangle_index, const Vec2& position);
    extern int AddVertex(MeshDocument* m, const Vec2& position);
    extern int RotateEdge(MeshDocument* m, int edge_index);
    extern void DrawMesh(MeshDocument* m, const Mat3& transform, Material* material=nullptr);
    extern Vec2 GetFaceCenter(MeshDocument* m, u16 face_index);
    inline Vec2 GetVertexPoint(MeshDocument* m, int vertex_index) {
        return GetCurrentFrame(m)->geom.verts[vertex_index].position;
    }
    extern void Update(MeshDocument* m, bool force=false);
    ...
    extern void SwapFace(MeshDocument* m, int face_index_a, int face_index_b);
    extern void SetOrigin(MeshDocument* m, const Vec2& origin);
    extern void SetBone(MeshDocument* m, int bone_index);  // Sets entire mesh to single bone (or clears if -1)
    extern void ClearBone(MeshDocument* m);  // Clears all bone weights from mesh

    extern void SetFaceColor(MeshDocument* m, u8 color);
    ...
    // @mesh
    inline bool IsSkinned(MeshDocument* mesh) {
        if (!mesh) return false;
        return mesh->skeleton_name && mesh->bone_index != U8_MAX;
    }
    ...
    extern u16 GetEdge(MeshDocument* m, u16 v0, u16 v1);
    ...
    // @geometry
    struct GeometryHitTestResult {
        u16 vertx_index = U16_MAX;
        u16 edge_index = U16_MAX;
        u16 face_index = U16_MAX;
    };
