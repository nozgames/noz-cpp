...
inline MeshDocument* GetMeshData() {
...
static int GetTotalAnimationFrames(MeshDocument* m) {
    int total = 0;
...
static int GetFrameForPlaybackTime(MeshDocument* m, int playback_frame) {
    int accumulated = 0;
    for (int i = 0; i < m->impl->frame_count; i++) {
...
static void RenameMesh(Document* a, const Name* new_name) {
...
static void MeshUndoRedo(Document* a) {
    assert(a->type == ASSET_TYPE_MESH);
    MeshDocument* m = static_cast<MeshDocument*>(a);
    MarkDirty(m);
