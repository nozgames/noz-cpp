...
static void Init(MeshDocument* m);
...
Mesh* ToOutlineMesh(MeshDocument* m) {
    MeshDataImpl* impl = m;
    MeshFrameData* frame = GetCurrentFrame(m);
...
MeshDocument* LoadMeshData(const std::filesystem::path& path) {
    ...
    MeshDocument* m = static_cast<MeshDocument*>(CreateAssetData(path));
    assert(m);
    Init(m);
    LoadMeshData(m);
    return m;
...
static void SaveMeshData(Document* a, const std::filesystem::path& path) {
    assert(a->type == ASSET_TYPE_MESH);
    MeshDocument* m = static_cast<MeshDocument*>(a);
...
static Document* CloneMeshDocument(Document* doc) {
    assert(doc->type == ASSET_TYPE_MESH);
    MeshDocument* mdoc = static_cast<MeshDocument*>(doc);


    MeshDataImpl* old_data = mdoc;
    AllocateData(mdoc);
    memcpy(mdoc, old_data, sizeof(MeshDataImpl));

    for (int i = 0; i < mdoc->frame_count; i++) {
        mdoc->frames[i].mesh = nullptr;
        mdoc->frames[i].outline = nullptr;
    }
    mdoc->playing = nullptr;
}

void InitMeshData(Document* doc) {
    assert(doc);
    assert(doc->type == ASSET_TYPE_MESH);
    MeshDocument* impl = static_cast<MeshDocument*>(doc);
    Init(impl);
}
...
void SetBone(MeshDocument* m, u8 bone_index) {
    MeshDataImpl* impl = m;
    m->bone_index = bone_index;
    Update(m);
    MarkDirty(m);
}
...
void Rasterize(
    ...
    RectInt rect = {
        position.x - padding,
        position.y - padding,
        position.x + m->size.x + padding * 2,
        position.y + m->size.y + padding * 2};

    Set(pixels, rect, COLOR32_TRANSPARENT);

    int palette_index = g_editor.palette_map[m->palette];
...
static void PostLoadMeshData(Document* a) {
    ...
    MeshDocument* m = static_cast<MeshDocument*>(a);
    if (m->skeleton_name)
        m->skeleton = static_cast<SkeletonDocument*>(GetDocument(ASSET_TYPE_SKELETON, m->skeleton_name));
