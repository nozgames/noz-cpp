...
namespace noz::editor {
    ...
    static AnimationDocument* GetAnimationData() {
        Document* doc = GetActiveDocument();
        assert(doc);
        assert(doc->def->type == ASSET_TYPE_ANIMATION);
        return static_cast<AnimationDocument*>(doc);
    }

    static SkeletonDocument* GetSkeletonData() { return GetAnimationData()->skeleton; }
    ...
    static Vec2 GetRootMotionOffset() {
        AnimationDocument* n = GetAnimationData();
    ...
    static bool IsAncestorSelected(int bone_index) {
        AnimationDocument* n = GetAnimationData();
        SkeletonDocument* s = GetSkeletonData();
    ...
    static bool TrySelectBone() {
        ...
        int hit[MAX_BONES];
        int hit_count = HitTestBones(n, GetBaseTransform(), g_workspace.mouse_world_position, hit, MAX_BONES);
        if (hit_count == 0) {
            if (!IsShiftDown())
                ClearSelection();
            return false;
    ...
    static void UpdateBoneNames() {
        ...
        if (!IsAltDown(g_animation_editor.input) && !g_workspace.show_names)
            return;
    ...
    static void UpdateDefaultState() {
        ...
        if (!g_animation_editor.ignore_up && !g_workspace.drag && WasButtonReleased(g_animation_editor.input, MOUSE_LEFT)) {
    ...
    static void DopeSheetFrame(AnimationDocument* n, int frame_index, int current_frame) {
    ...
    static void DopeSheet() {
        AnimationDocument* adoc = GetAnimationData();
        int frame_count = Max(GetFrameCountWithHolds(adoc), DOPESHEET_MIN_FRAMES);
        ...
        // Ticks
        BeginRow();
        bool playing = IsPlaying(adoc->animator);
        int current_frame = playing
            ? GetFrameIndex(adoc->animator)
            : adoc->current_frame;
        ...
        for (int frame_index=0; frame_index<=frame_count; frame_index++) {
            ...
            int real_frame_index = GetRealFrameIndex(adoc, frame_index);
            if (WasPressed()) {
                adoc->current_frame = real_frame_index;
        ...
        BeginRow();
        {
            ...
            current_frame = IsPlaying(adoc->animator)
                ? GetRealFrameIndex(adoc, GetFrameIndex(adoc->animator))
                : adoc->current_frame;
            for (frame_index = 0; frame_index<adoc->frame_count; frame_index++) {
                AnimationFrameData* f = &adoc->frames[frame_index];
                frame_index_with_holds += 1 + f->hold;
                DopeSheetFrame(adoc, frame_index, current_frame);
            }
    ...
    static void Inspector() {
        ...
        EventDocument* events[EDITOR_MAX_DOCUMENTS];
        ...
        for (int asset_index=0, asset_count=GetDocumentCount(); asset_index<asset_count; asset_index++) {
            ...
            if (a->def->type != ASSET_TYPE_EVENT) continue;
            if (a->name == frame.event_name)
                current_event_index = event_count + 1;
            events[event_count++] = static_cast<EventDocument*>(a);
        }
    ...
    static void DrawOnionSkin() {
        ...
        BindMaterial(g_workspace.shaded_skinned_material);
    ...
    void DrawAnimationEditor() {
        ...
        for (int i=0; i<s->skin_count; i++) {
            MeshDocument* skinned_mesh = s->skins[i].mesh;
            if (!skinned_mesh)
                continue;
    ...
    static void BeginAnimationEditor(Document*) {
        ClearSelection();
        SetDefaultState();
    ...
    void InitAnimationEditor(AnimationDocument* n) {
