...
extern void InitSkeletonEditor(SkeletonDocument* s);
...
static void BuildSkeletonDisplayMesh(SkeletonDocument* s, const Vec2& position) {
...
void DrawSkeletonData(SkeletonDocument* s, const Vec2& position) {
    SkeletonDataImpl* impl = s->impl;
...
static void DrawSkeletonData(Document* a) {
    assert(a);
...
int HitTestBones(SkeletonDocument* s, const Mat3& transform, const Vec2& position, int* bones, int max_bones) {
    SkeletonDataImpl* impl = s->impl;
    int hit_count = 0;
...
int HitTestBone(SkeletonDocument* s, const Mat3& transform, const Vec2& position) {
    SkeletonDataImpl* impl = s->impl;
    int bones[MAX_BONES];
...
int HitTestBone(SkeletonDocument* s, const Vec2& world_pos) {
...
static void ParseBone(SkeletonDocument* s, Tokenizer& tk) {
    SkeletonDataImpl* impl = s->impl;
...
static void LoadSkeletonData(Document* a) {
    assert(a);
    assert(a->type == ASSET_TYPE_SKELETON);
    SkeletonDocument* s = static_cast<SkeletonDocument*>(a);
...
static void SaveSkeletonData(Document* a, const std::filesystem::path& path) {
    assert(a);
    assert(a->type == ASSET_TYPE_SKELETON);
...
Document* NewSkeletonDocument(const std::filesystem::path& path) {
    ...
    Document* a = CreateAssetData(full_path);
    if (a && a->vtable.load)
        a->vtable.load(a);
    return a;
}

void UpdateTransforms(SkeletonDocument* s) {
    SkeletonDataImpl* impl = s->impl;
...
static void LoadSkeletonMetaData(Document* a, Props* meta) {
    assert(a);
    assert(a->type == ASSET_TYPE_SKELETON);
...
static void PostLoadSkeleton(Document* a) {
    assert(a);
    assert(a->type == ASSET_TYPE_SKELETON);
    SkeletonDocument* s = static_cast<SkeletonDocument*>(a);
    SkeletonDataImpl* impl = s->impl;
    ...
    for (int i=0; i<impl->skin_count; i++) {
        ...
        sm.mesh = static_cast<MeshData*>(GetDocument(ASSET_TYPE_MESH, sm.asset_name));
        if (!sm.mesh)
            continue;
...
int FindBoneIndex(SkeletonDocument* s, const Name* name) {
    SkeletonDataImpl* impl = s->impl;
    for (int i=0; i<impl->bone_count; i++)
...
int ReparentBone(SkeletonDocument* s, int bone_index, int parent_index) {
    SkeletonDataImpl* impl = s->impl;
...
int GetBoneSide(SkeletonDocument* s, int bone_index) {
    SkeletonDataImpl* impl = s->impl;
...
int GetMirrorBone(SkeletonDocument* s, int bone_index) {
    SkeletonDataImpl* impl = s->impl;
...
void RemoveBone(SkeletonDocument* s, int bone_index) {
    SkeletonDataImpl* impl = s->impl;
...
const Name* GetUniqueBoneName(SkeletonDocument* s) {
...
void Serialize(SkeletonDocument* s, Stream* stream) {
    SkeletonDataImpl* impl = s->impl;
...
Skeleton* ToSkeleton(Allocator* allocator, SkeletonDocument* s) {
...
static void SaveSkeletonMetadata(Document* a, Props* meta) {
    assert(a);
    assert(a->type == ASSET_TYPE_SKELETON);
    SkeletonDocument* s = (SkeletonDocument*)a;
    SkeletonDataImpl* impl = s->impl;
...
static void SkeletonUndoRedo(Document* a) {
    assert(a);
    assert(a->type == ASSET_TYPE_SKELETON);
    SkeletonDocument* s = (SkeletonDocument*)a;
    UpdateTransforms(s);
    ...
    for (u32 i = 0, c = GetDocumentCount(); i < c; i++) {
        AnimationData* anim = static_cast<AnimationData*>(GetDocument(i));
...
static void AllocateSkeletonImpl(Document* a) {
    assert(a->type == ASSET_TYPE_SKELETON);
    SkeletonDocument* s = static_cast<SkeletonDocument*>(a);
...
static void CloneSkeletonData(Document* a) {
    assert(a->type == ASSET_TYPE_SKELETON);
    SkeletonDocument* s = static_cast<SkeletonDocument*>(a);
...
static void DestroySkeletonData(Document* a) {
    SkeletonDocument* s = static_cast<SkeletonDocument*>(a);
    assert(s);
...
static void InitSkeletonData(SkeletonDocument* s) {
    assert(s);
...
void InitSkeletonData(Document* a) {
    assert(a);
    assert(a->type == ASSET_TYPE_SKELETON);
    InitSkeletonData(static_cast<SkeletonDocument*>(a));
}
