...
namespace noz::lua {
    ...
    ByteCode CompileLua(const char* code) {
        size_t size = 0;
    ...
    State* CreateState(Allocator* allocator) {
    ...
    void Load(State* state, Script* script) {
        StateImpl* state_impl = static_cast<StateImpl*>(state);
    ...
    void SetGlobal(State* state, const char* name, Asset* asset) {
        StateImpl* state_impl = static_cast<StateImpl*>(state);
