...
namespace noz
{
	...
	int RectPacker::Insert(const glm::ivec2& size, Method method, BinRect& result) {
		BinRect rect;
		int32_t score1;
		int32_t score2;
		...
		switch (method) 
		{
		case Method::BestShortSideFit:  rect = FindPositionForNewNodeBestShortSideFit(size.x, size.y, score1, score2); break;
		case Method::BottomLeftRule:    rect = FindPositionForNewNodeBottomLeft(size.x, size.y, score1, score2); break;
		case Method::ContactPointRule:  rect = FindPositionForNewNodeContactPoint(size.x, size.y, score1); break;
		case Method::BestLongSideFit:   rect = FindPositionForNewNodeBestLongSideFit(size.x, size.y, score2, score1); break;
		case Method::BestAreaFit:       rect = FindPositionForNewNodeBestAreaFit(size.x, size.y, score1, score2); break;
		}
	...
	/*
	void RectPacker::Insert(std::vector<RectSize> &rects, std::vector<Rect> &dst, Method method)
	{
		dst.clear();

		while(rects.size() > 0)
		{
			int32_t bestScore1 = std::numeric_limits<int32_t>::max();
			int32_t bestScore2 = std::numeric_limits<int32_t>::max();
			int32_t bestRectIndex = -1;
			Rect rect;
	...
	*/
	...
	void RectPacker::PlaceRect(const BinRect& rect) 
	{
	...
	RectPacker::BinRect RectPacker::ScoreRect(BinSize size, Method method, int32_t& score1, int32_t& score2) const {
		BinRect rect;
	...
	float RectPacker::GetOccupancy(void) const {
	...
	RectPacker::BinRect RectPacker::FindPositionForNewNodeBottomLeft(int32_t width, int32_t height, int32_t& bestY, int32_t& bestX) const {
		BinRect rect;
	...
	RectPacker::BinRect RectPacker::FindPositionForNewNodeBestShortSideFit(int32_t width, int32_t height, int32_t& bestShortSideFit, int32_t& bestLongSideFit) const {
		BinRect rect;
		...
		for (size_t i = 0; i < free_.size(); ++i)
		{
			...
			if (free_[i].w >= height && free_[i].h >= width)
			{
				...
				int32_t flippedShortSideFit = math::min(flippedLeftoverHoriz, flippedLeftoverVert);
				int32_t flippedLongSideFit = math::max(flippedLeftoverHoriz, flippedLeftoverVert);
	...
	RectPacker::BinRect RectPacker::FindPositionForNewNodeBestLongSideFit(int32_t width, int32_t height,
		int32_t& bestShortSideFit, int32_t& bestLongSideFit) const
	{
		BinRect rect;
		memset(&rect, 0, sizeof(rect));
	...
	RectPacker::BinRect RectPacker::FindPositionForNewNodeBestAreaFit(int32_t width, int32_t height,
		int32_t& bestAreaFit, int32_t& bestShortSideFit) const
	{
		BinRect rect;
		memset(&rect, 0, sizeof(Rect));
	...
	RectPacker::BinRect RectPacker::FindPositionForNewNodeContactPoint(int32_t width, int32_t height, int32_t& bestContactScore) const {
		BinRect rect;
		memset(&rect, 0, sizeof(Rect));
	...
	bool RectPacker::SplitFreeNode(BinRect freeNode, const BinRect& usedNode) {
	...
	void RectPacker::PruneFreeList(void) {
