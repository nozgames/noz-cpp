...
namespace noz::import::ttf
{
	...
	TrueTypeFontReader::TrueTypeFontReader(StreamReader* reader, int requestedSize, const std::string& filter)
		: _reader(reader),
	...
	{
		auto bytes = _reader->readBytes(length);
	...
	{
		return (uint16_t)((_reader->readByte() << 8) | _reader->readByte());
	}

	int16_t TrueTypeFontReader::readInt16()
	{
		return (int16_t)((_reader->readByte() << 8) | _reader->readByte());
	}

	uint32_t TrueTypeFontReader::readUInt32()
	{
		return
			(((uint32_t)_reader->readByte()) << 24) |
			(((uint32_t)_reader->readByte()) << 16) |
			(((uint32_t)_reader->readByte()) << 8) |
			_reader->readByte();
	}
	...
	{
		int64_t old = (int64_t)_reader->position();
		_reader->seekBegin(offset);
		return old;
	}
	...
	void TrueTypeFontReader::readCMAP()
	{
		...
		switch (format)
		{
			case 4:
			{
				...
				auto glyphIdArray = _reader->position();
	...
	void TrueTypeFontReader::readGlyph(TrueTypeFont::Glyph& glyph)
	{
		...
		auto instructions = _reader->readBytes(instructionLength);
	...
	void TrueTypeFontReader::readKERN()
	{
		...
		{
			long tableStart = (long)_reader->position();
			/*auto version = */
			readUInt16();
